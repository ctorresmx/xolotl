...
mod tests {
    ...
    async fn test_cleanup_all_unhealthy_services() {
        ...
        // Add unhealthy services (old heartbeats)
        {
            let mut reg = registry.write().await;
            let service1 =
                create_test_service("service1", "prod", config.stale_threshold_ms + 1000);
            let service2 =
                create_test_service("service2", "prod", config.stale_threshold_ms + 2000);
            reg.register(service1).unwrap();
            reg.register(service2).unwrap();
        }
    ...
    async fn test_cleanup_mixed_health_services() {
        ...
        // Add mixed services
        {
            ...
            let stale_service =
                create_test_service("stale", "prod", config.healthy_threshold_ms + 1000);
            let unhealthy_service =
                create_test_service("unhealthy", "prod", config.stale_threshold_ms + 1000);
        ...
        let service_names: Vec<&str> = remaining_services
            .iter()
